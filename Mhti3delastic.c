/* 3D three-components elastic wavefield extrapolation (CPML boundary condition)
   ...
   equation in heterogeneous HTI media. In order to adapt large time step, we use
   k-space adjustment.
...
int main(int argc, char* argv[])
{
   ...
   /* setup I/O files */
    sf_file Fvp0,Fvs0,Fepsi,Fdel,Fgam,Fphi;
  ...
  /* puthead3x(Fo1,nz-2*nzpad,nx-2*nxpad,ny-2*nypad,dz/1000.0,dx/1000.0,dy/1000.,0,0,0);
   ...
   puthead3x(Fo1,nz-2*0,nx-2*0,ny-2*0,dz/1000.0,dx/1000.0,dy/1000.,0,0,0);
   puthead3x(Fo2,nz-2*0,nx-2*0,ny-2*0,dz/1000.0,dx/1000.0,dy/1000.,0,0,0);
   puthead3x(Fo3,nz-2*0,nx-2*0,ny-2*0,dz/1000.0,dx/1000.0,dy/1000.,0,0,0);
   puthead3x(Fo4,nz-2*0,nx-2*0,ny-2*0,dz/1000.0,dx/1000.0,dy/1000.,0,0,0);
   puthead3x(Fo5,nz-2*0,nx-2*0,ny-2*0,dz/1000.0,dx/1000.0,dy/1000.,0,0,0);
   puthead3x(Fo6,nz-2*0,nx-2*0,ny-2*0,dz/1000.0,dx/1000.0,dy/1000.,0,0,0);
   puthead3x(Fo7,nz-2*0,nx-2*0,ny-2*0,dz/1000.0,dx/1000.0,dy/1000.,0,0,0);
   puthead3x(Fo8,nz-2*0,nx-2*0,ny-2*0,dz/1000.0,dx/1000.0,dy/1000.,0,0,0);
   puthead3x(Fo9,nz-2*0,nx-2*0,ny-2*0,dz/1000.0,dx/1000.0,dy/1000.,0,0,0);
   ...
   // elastic parameter
   float *c11,*c12,*c13,*c16,*c22,*c23,*c26,
   ...
   float *vp0,*vs0,*epsi,*del,*gam,*phi;
   ...
   Thomson2stiffness_3dhti(vp0,vs0,epsi,del,gam,phi,c11,c12,c13,c16,c22,
		                   c23,c26,c33,c36,c44,c45,c55,c66,nx,ny,nz);
   int nnz = nz*7/12;
   float vp = 3578.;
   float vs = 1878.;
   float ep1 = 0.1822;
   ...
   for(iz=nnz;iz<nz;iz++)
   {
       ...
       c33[i] = vp*vp;
       c55[i] = vs*vs;
       c11[i] = c33[i]*(1.0+2.0*ep2);
       c13[i] = sqrt((c33[i]-c55[i])*(c33[i]-c55[i]+2.*c33[i]*de2))-c55[i];
       c22[i] = (2.*ep1 + 1.)*c33[i];
       c66[i] = (2.*ga1+1.0)*c55[i];
       c44[i] = (2.*ga2+1.0)*c66[i];
       c12[i] = sqrt((c11[i]-c66[i])*(c11[i]-c66[i]+2.*c11[i]*de3))-c66[i];
       c23[i] = sqrt((c33[i]-c44[i])*(c33[i]-c44[i]+2.*c33[i]*de1))-c44[i];
   }
...
*/
	  #pragma omp parallel for private(iy,ix,iz,i) schedule(dynamic)\
		   shared(Txx1,Txx2,dt,c11,c12,c13,c16,c22,c23,c26,c33,c36,c44,c45,c55,c66,d1,d2,\
	  ...
	  for(iy=0;iy<ny;iy++)
	   ...
	   {
		   i = iy*nx*nz+ix*nz+iz;
           Txx2[i] = Txx1[i] + dt*(c11[i]*d1[i]+c12[i]*d5[i]+c13[i]*d9[i]+c16[i]*(d2[i]+d4[i]));

           Tyy2[i] = Tyy1[i] + dt*(c12[i]*d1[i]+c22[i]*d5[i]+c23[i]*d9[i]+c26[i]*(d2[i]+d4[i]));

           Tzz2[i] = Tzz1[i] + dt*(c13[i]*d1[i]+c23[i]*d5[i]+c33[i]*d9[i]+c36[i]*(d2[i]+d4[i]));

           Tyz2[i] = Tyz1[i] + dt*(c44[i]*(d6[i]+d8[i])+c45[i]*(d3[i]+d7[i]));

           Txz2[i] = Txz1[i] + dt*(c45[i]*(d6[i]+d8[i])+c55[i]*(d3[i]+d7[i]));

           Txy2[i] = Txy1[i] + dt*(c16[i]*d1[i]+c26[i]*d5[i]+c36[i]*d9[i]+c66[i]*(d2[i]+d4[i]));

		   if(ix<nxpad || ix>nx-nxpad-1)
		   {
			   Txx2[i] += dt*(c11[i]*phi_vx_x2[i] + c16[i]*phi_vy_x2[i]); 
			   Tyy2[i] += dt*(c12[i]*phi_vx_x2[i] + c26[i]*phi_vy_x2[i]);
			   Tzz2[i] += dt*(c13[i]*phi_vx_x2[i] + c36[i]*phi_vy_x2[i]);
			   Tyz2[i] += dt*(c45[i]*phi_vz_x2[i]);
			   Txz2[i] += dt*(c55[i]*phi_vz_x2[i]);
			   Txy2[i] += dt*(c16[i]*phi_vx_x2[i]+ c66[i]*phi_vy_x2[i]);
		   ...
		   {
			   Txx2[i] += dt*(c16[i]*phi_vx_y2[i] + c12[i]*phi_vy_y2[i]); 
			   Tyy2[i] += dt*(c26[i]*phi_vx_y2[i] + c22[i]*phi_vy_y2[i]);
			   Tzz2[i] += dt*(c36[i]*phi_vx_y2[i] + c23[i]*phi_vy_y2[i]);
			   Tyz2[i] += dt*(c44[i]*phi_vz_y2[i]);
			   Txz2[i] += dt*(c45[i]*phi_vz_y2[i]);
			   Txy2[i] += dt*(c66[i]*phi_vx_y2[i] + c26[i]*phi_vy_y2[i]);
		   ...
		   if(iz<nzpad || iz>nz-nzpad-1)
		   {
			   ...
			   Tyz2[i] += dt*(c45[i]*phi_vx_z2[i] + c44[i]*phi_vy_z2[i]);
			   Txz2[i] += dt*(c55[i]*phi_vx_z2[i] + c45[i]*phi_vy_z2[i]);
			   Txy2[i] += dt*(c36[i]*phi_vz_z2[i]); 
	   ...
	   {
		    sf_warning("k=%d",k);
			sf_warning("vx=%f",vx2[nx*ny*nz/2]);
	   }
