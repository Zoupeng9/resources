        ...
        if(it%10==0)
         ...
         for(j=-1;j<=1;j++)
         {
             if(fabs(i)+fabs(j)==2)
             {
                  ...
                  if(it%10==0) sf_warning("ux=%f uz=%f ",ux[(ixs+i)*nz+(izs+j)],uz[(ixs+i)*nz+(izs+j)]);
             }
        }
...
int sampleopx1(vector<int>& rs, vector<int>& cs, DblNumMat& resx)
{
    ...
    double   a11, a12, a22;
    double  sx, cx, sx2, cx2, kxz, ax, axz, u1, u2;
    ...
    for(int a=0; a<nr; a++)
    {
        ...
        for(int b=0; b<nc; b++)
        {
            ...
            a11= c11[i]*sx*sx+c55[i]*cx*cx+2.0*c15[i]*sx*cx;
            a12= (c13[i]+c55[i])*sx*cx+c15[i]*sx*sx+c35[i]*cx*cx;
            a22= c55[i]*sx*sx+c33[i]*cx*cx+2.0*c35[i]*sx*cx;
            ...
            double dt2=dt1*dt1;
            ax = 2.0-dt2*(c11[i]*sx2+c55[i]*cx2+2.0*c15[i]*kxz);
            axz = dt2*((c13[i]+c55[i])*kxz+c15[i]*sx2+c35[i]*cx2);
...
int sampleopx2(vector<int>& rs, vector<int>& cs, DblNumMat& resx)
{
    ...
    double  aa[2][2],ve[2][2],va[2];  /*matrix, eigeinvector and eigeinvalues*/
    double   a11, a12, a22;
    double  sx, cx, sx2, cx2, kxz, az, axz, u1, u2;
    ...
    for(int a=0; a<nr; a++)
    {
        ...
        for(int b=0; b<nc; b++)
        {
            ...
            // wavefield extrapolator
            double dt2=dt1*dt1;
            az = 2.0-dt2*(c55[i]*sx2+c33[i]*cx2+2.0*c35[i]*kxz);
            axz = dt2*((c13[i]+c55[i])*kxz+c15[i]*sx2+c35[i]*cx2);
