/* 3D three-components elastic wavefield extrapolation (CPML boundary condition)
   ...
   equation in heterogeneous VTI media. In order to adapt large time step, we use
   k-space adjustment.
...
int main(int argc, char* argv[])
{
   ...
   /* setup I/O files */
    sf_file Fvp0,Fvs0,Fepsi,Fdel,Fgam;
	...
	Fepsi = sf_input("epsi");
	Fdel = sf_input("del");
	Fgam = sf_input("gam");// Thomosen parameters for anisotropic media
   ...
   for(i=0;i<nxyz;i++)
   {
	   ...
	   c33[i] = vp0[i]*vp0[i];
	   c44[i] = vs0[i]*vs0[i];
	   c11[i] = c33[i]*(1.0+2.0*epsi[i]);
	   c13[i] = sqrt((c33[i]-c44[i])*(c33[i]-c44[i]+2.*c33[i]*del[i]))-c44[i];
	   c66[i] = (2.*gam[i]+1.0)*c44[i];
	   c55[i] = c44[i];
	   c12[i] = c11[i] - 2.*c66[i];
	   c22[i] = c11[i];
	   c23[i] = c13[i];
