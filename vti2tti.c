...
/*< Thomson3stiffness_3d: convert Thomson parameters to stiffness matrix in tti media >*/
{
	...
	for(ixyz=0;ixyz<nxyz;ixyz++)
	{
		...
		if((c_11-c_66)*c_33<c_13*c_13)
			sf_warning("The stiffness matrix not positive");
...
void Thomson2stiffness_3dhti(float* vp0, float* vs0, float* ep,  float* de, float* ga,float* ph,float* c11,float* c12,
                          float* c13, float* c16,float* c22,float* c23,float* c26,float* c33, float* c36,float* c44,
						  float* c45,float* c55,float* c66, int nx, int ny, int nz)
/*< Thomson3stiffness_3d: convert Thomson parameters to stiffness matrix in hti media >*/
{
	int   i,j,k,ixyz,nxyz;
	nxyz = nx*ny*nz;
	float c_11,c_13,c_33,c_44,c_55;
	float a11,a12,a13,a21,a22,a23,a31,a32,a33;
	...
	for(ixyz=0;ixyz<nxyz;ixyz++)
	{
		...
		c_55 = vs0[ixyz]*vs0[ixyz];
		c_11 = c_33*(1.0 + 2.0*ep[ixyz]);
		c_13 = sqrt((c_33 - c_55)*(c_33 - c_55 + 2.0*c_33*de[ixyz])) - c_55;
		c_44 = 1.0/(2.0*ga[ixyz]+1.0)*c_55;
