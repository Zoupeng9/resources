...
int main(int argc, char* argv[])
{
   ...
   int i,j,k;
//   FILE *fffpp;
//   fffpp = fopen("/data/zp/graduation/viscoacoustic/vel.dat","rb");
   ...
   int nxv, nzv;
   float dx,dz,fx,fz;
   az = sf_iaxa(Fi1,1); nzv = sf_n(az); dz = sf_d(az)*1000;
   ax = sf_iaxa(Fi1,2); nxv = sf_n(ax); dx = sf_d(ax)*1000;
   fz = sf_o(az)*1000.0;
   fx = sf_o(ax)*1000.0;
	   ...
	   for(j=0;j<nx;j++)
	   {
        ...
        for(k=0;k<nt;k++)
        {
			...
			else
			{
                ...
                p2[z0][x0] += Ricker(k*dt,f0,0.1,10);
