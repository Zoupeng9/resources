...
int lrosback2(sf_complex **img, sf_complex ***wavfld, float **sill, sf_complex **rcdf, sf_complex **rcd, bool adj,
	      bool verb, bool wantwf, sf_complex **lt, sf_complex **rt, int m2,
              geopar geop, int pad1, bool illum, sf_complex **ltad, sf_complex **rtad, int m2ad)
/*< low-rank one-step backward propagation + imaging >*/
{
	...
	sf_complex *cwavead, *cwavemad;
    sf_complex **wave, *curr;
    sf_complex **wavead, *currad;
    sf_complex **ccr;
...
/*     dx = geop->dx;
    ...
    curr = sf_complexalloc(nzx2);
    currad = sf_complexalloc(nzx2);
    cwave  = sf_complexalloc(nk);
    cwavead  = sf_complexalloc(nk);
    wave = sf_complexalloc2(nzx2,m2);
    wavead = sf_complexalloc2(nzx2,m2ad);
...
#pragma omp parallel for private(ix,iz,j)
	  ...
	  for (ix=0; ix<nx; ix++) {
	    for (iz=0; iz<nz; iz++) {
	      j = (iz+geop->top)+(ix+geop->lft)*nz2; /* padded grid */
		  currad[j] = wavfld[wfit][ix][iz];
		}
	  }

	cwavemad = sf_complexalloc(nk);
	//icfft2_allocate(cwavemad);
    /*matrix multiplication*/
    cfft2(currad,cwave);
    for (im = 0; im < m2ad; im++) {
#ifdef _OPENMP
#pragma omp parallel for private(ik)
      ...
      for (ik = 0; ik < nk; ik++) {
#ifdef SF_HAS_COMPLEX_H
        cwavemad[ik] = cwave[ik]*rtad[ik][im];
#else
        cwavemad[ik] = sf_cmul(cwave[ik],rtad[ik][im]);
#endif
      }
      icfft2(wavead[im],cwavemad);
    }
...
#pragma omp parallel for private(ix,iz,i,j,im,c) shared(curr,lt,wave)
    ...
    for (ix = 0; ix < nxb; ix++) {
      for (iz=0; iz < nzb; iz++) {
        ...
        for (im = 0; im < m2ad; im++) {
#ifdef SF_HAS_COMPLEX_H
          c += ltad[im][i]*wavead[im][j];
#else
          c = sf_cadd(c,sf_cmul(ltad[im][i], wavead[im][j]));
#endif
        }
        currad[j] = c;
      }
...
#pragma omp parallel for private(ix,iz,j)
	  ...
	  for (ix=0; ix<nx; ix++) {
	    for (iz=0; iz<nz; iz++) {
	      j = (iz+geop->top)+(ix+geop->lft)*nz2; /* padded grid */
#ifdef SF_HAS_COMPLEX_H
	      ccr[ix][iz] += conjf(currad[j])*curr[j];
	     // ccr[ix][iz] += conjf(wavfld[wfit][ix][iz])*curr[j];
#else
	      ccr[ix][iz] = sf_cadd(ccr[ix][iz],sf_cmul(conjf(currad[j]),curr[j]));
#endif
	    }
	  }
	  wfit--;
...
#pragma omp parallel for private(ix, iz)
    ...
    cfft2_finalize();
//	free(cwave); free(cwavem); free(currm);
	free(curr); free(currad); 
	free(*wave); free(wave);
	free(*wavead); free(wavead);
	free(*ccr); free(ccr);
    return 0;
}
...
int main(int argc, char* argv[]) 
{
    ...
    int nx, nz, nt, wfnt;
    int nzx, nx2, nz2, n2, m2, m2b, m2ad, pad1, nk;
    int ix, iz, it, is;
    int nxb, nzb;
    int snpint;
    float dt, dx, dz, wfdt;
    ...
    sf_complex **lt, **rt;
    sf_complex **ltb, **rtb;
    sf_complex **ltad, **rtad; //adjoint for calculating gradient
    ...
    leftad  = sf_input("leftad");
    rightad = sf_input("rightad");
    ...
    if (!sf_histint(leftad,"n1",&n2) || n2 != nzx) sf_error("Need n1=%d in left",nzx);
    if (!sf_histint(leftad,"n2",&m2ad))  sf_error("Need n2= in left");
    if (!sf_histint(rightad,"n1",&n2) || n2 != m2ad) sf_error("Need n1=%d in right",m2);
    if (!sf_histint(rightad,"n2",&n2) || n2 != nk) sf_error("Need n2=%d in right",nk);
    ...
    ltad = sf_complexalloc2(nzx,m2ad);
    rtad = sf_complexalloc2(m2ad,nk);
...
#pragma omp parallel for private(ix,iz)
    ...
    sf_complexread(ltad[0],nzx*m2ad,leftad);
    sf_complexread(rtad[0],m2ad*nk,rightad);
    if (rank==0) {
...
#pragma omp parallel for private(is,ix,it)
	...
	MPI_Scatter(sendbuf, gpl*nt, MPI_COMPLEX, recvbuf, gpl*nt, MPI_COMPLEX, 0, MPI_COMM_WORLD); // tmprecf[ix][it] = record[is][ix][it];
      ...
      if (shtcur<shtnum0) {
	lrosback2(img, wavefld, sill, tmprecf, tmprec, adj, verb, wantwf, ltb, rtb, m2b, geop, pad1, illum, ltad, rtad, m2ad);
