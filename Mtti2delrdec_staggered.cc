...
#include "eigen2x2.h"
#include "decomplowrank.stagger.h"
#include "seplowrank.h"  // for sub-program: reconstruct(...)
}

static std::valarray<float> vp, vs, ep, de, th;

static std::valarray<double> sinx, cosx,rkx,rkz,rkk;
static double dxxh,dzzh;

/* dual-domain wave-mode separation and wave vector decomposition operator based on low-rank decomp. */
static int samplexx(vector<int>& rs, vector<int>& cs, ZpxNumMat& resx);
static int samplexz(vector<int>& rs, vector<int>& cs, ZpxNumMat& resx);
static int samplezx(vector<int>& rs, vector<int>& cs, ZpxNumMat& resx);
static int samplezz(vector<int>& rs, vector<int>& cs, ZpxNumMat& resx);

static void map2d1d(sf_complex *d, ZpxNumMat mat, int m, int n);
/*****************************************************************************************/
int main(int argc, char* argv[])
{
   ...
   * ***************************************************************************/
   ...
   int   m2xx, n2xx, m2xz, n2xz,m2zx,n2zx,m2zz, n2zz;
   sf_complex *ldataxx, *fmidxx, *rdataxx;
   sf_complex *ldataxz, *fmidxz, *rdataxz;
   sf_complex *ldatazx, *fmidzx, *rdatazx;
   sf_complex *ldatazz, *fmidzz, *rdatazz;
   ...
   fmidxx  = sf_complexalloc(m2xx*n2xx);
   ldataxx = sf_complexalloc(nxz*m2xx);
   rdataxx = sf_complexalloc(n2xx*nk);
  ...
  /** ApxApz ****/
   ...
   fmidxz  = sf_complexalloc(m2xz*n2xz);
   ldataxz = sf_complexalloc(nxz*m2xz);
   rdataxz = sf_complexalloc(n2xz*nk);
  ...
  /** ApzApx ****/
   iC( ddlowrank(nxz,nk,samplezx,eps,npk,lid,rid,mid) );
   m2zx=mid.m();
   n2zx=mid.n();
   sf_warning("m2xz=%d n2xz=%d",m2xz, n2xz);

   fmidzx  = sf_complexalloc(m2zx*n2zx);
   ldatazx = sf_complexalloc(nxz*m2zx);
   rdatazx = sf_complexalloc(n2zx*nk);

   map2d1d(fmidzx, mid, m2zx, n2zx);

   iC ( samplezx(md,lid,mat) );
   map2d1d(ldatazx, mat, nxz, m2zx);

   iC ( samplezx(rid,nd,mat) );
   map2d1d(rdatazx, mat, n2zx, nk);
   ...
   fmidzz  = sf_complexalloc(m2zz*n2zz);
   ldatazz = sf_complexalloc(nxz*m2zz);
   rdatazz = sf_complexalloc(n2zz*nk);
   ...
   Orthog.put("o2",fx/1000);
    ...
    decomplowrank2d_stagger(ldataxx, rdataxx, fmidxx, 
                             ldataxz, rdataxz, fmidxz,                          
                             ldatazx, rdatazx, fmidzx,                          
                             ldatazz, rdatazz, fmidzz,
                             px, pz, ijkx, ijkz,
                             nx, nz, nxz, nk, M, m2xx, n2xx, m2xz, n2xz,m2zx,n2zx,m2zz, n2zz);

    sf_warning("vector decomposition of SV-wave based on lowrank decomp."); 
    decomplowrank2ds_stagger(ldataxx, rdataxx, fmidxx, 
                              ldataxz, rdataxz, fmidxz,                              
                              ldatazx, rdatazx, fmidzx,                              
                              ldatazz, rdatazz, fmidzz,
                              px1, pz1, ijkx, ijkz,
...
static int samplexz(vector<int>& rs, vector<int>& cs, ZpxNumMat& resx)
{
    ...
    for(int a=0; a<nr; a++) 
    {
        ...
        for(int b=0; b<nc; b++)
        {
			...
			double angle=kx*dxxh-kz*dzzh;
            resx(a,b) = zpx(u1*u2*cos(angle),u1*u2*sin(angle));
...
static void map2d1d(sf_complex *d, ZpxNumMat mat, int m, int n)
{
   ...
   for (j=0; j < n; j++)
   {
	   d[k] = sf_cmplx((float)(real(mat(i,j))), (float)(imag(mat(i,j))));
        k++;
   }
