...
int main(int argc, char* argv[])
{
   ...
   free(de);
       ...
       if(nz%2==0)
       {
		...
		for(i=0;i<nz;i++)
			for(j=0;j<nx;j++)
			{
				f_sta_x[i*nx+j] = I*kx[j];//*cexpf(I*(kx[j]*dx-kz[i]*dz)/2.0);
				b_sta_x[i*nx+j] = I*kx[j];//*cexpf(-I*(kx[j]*dx-kz[i]*dz)/2.0);
				f_sta_z[i*nx+j] = I*kz[i];//*cexpf(I*(kx[j]*dx-kz[i]*dz)/2.0);
				b_sta_z[i*nx+j] = I*kz[i];//*cexpf(-I*(kx[j]*dx-kz[i]*dz)/2.0);
			}

   t3=clock();
   for(k=0;k<nt;k++)
   {
	   RstaggerPS1order2Ddx3(zvx2,d1,nx,nz,f_sta_x,b_sta_x,f_sta_z,b_sta_z,0,1); //dvx/dx 
	   RstaggerPS1order2Ddx3(zvx2,d2,nx,nz,f_sta_x,b_sta_x,f_sta_z,b_sta_z,1,1); //dvx/dz 
	   RstaggerPS1order2Ddx3(zvz2,d3,nx,nz,f_sta_x,b_sta_x,f_sta_z,b_sta_z,1,1); //dvz/dz 
	   RstaggerPS1order2Ddx3(zvz2,d4,nx,nz,f_sta_x,b_sta_x,f_sta_z,b_sta_z,0,1); //dvz/dx 
	   ...
	   for(i=0;i<nz;i++)
		   for(j=0;j<nx;j++)
		   {
			/*	   zTxx2[i][j] = dt*(c13[i][j]*d3[i][j]+c15[i][j]*d2[i][j]) + zTxx1[i][j];
			   ...
			   zTxx2[i][j] = dt*(c13[i][j]*d3[i][j]+c15[i][j]*d2[i][j]+c11[i][j]*d1[i][j]+c15[i][j]*d4[i][j]) + zTxx1[i][j];
			   zTzz2[i][j] = dt*(c33[i][j]*d3[i][j]+c35[i][j]*d2[i][j]+c13[i][j]*d1[i][j]+c35[i][j]*d4[i][j]) + zTzz1[i][j];
			   zTxz2[i][j] = dt*(c55[i][j]*d2[i][j]+c35[i][j]*d3[i][j]+c55[i][j]*d4[i][j]+c15[i][j]*d1[i][j]) + zTxz1[i][j];
		   }
	 ...
	 //  xTzz2[nz/2][nx/2] += Ricker(k*dt,20,0.04,10); //source term
	   ...
	   RstaggerPS1order2Ddx3(zTxx2,d1,nx,nz,f_sta_x,b_sta_x,f_sta_z,b_sta_z,0,0); //dTxx/dx 
	   RstaggerPS1order2Ddx3(zTzz2,d2,nx,nz,f_sta_x,b_sta_x,f_sta_z,b_sta_z,1,0); //dTzz/dz 
	   RstaggerPS1order2Ddx3(zTxz2,d3,nx,nz,f_sta_x,b_sta_x,f_sta_z,b_sta_z,1,0); //dTxz/dz 
	   RstaggerPS1order2Ddx3(zTxz2,d4,nx,nz,f_sta_x,b_sta_x,f_sta_z,b_sta_z,0,0); //dTxz/dx 
   ...
   time_eachstep = (float)(t2 - t3)/CLOCKS_PER_SEC/nt*1000;
   sf_warning("costime %fs",timespent);
   sf_warning("each step costime %fms",time_eachstep);
