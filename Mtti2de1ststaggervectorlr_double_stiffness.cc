...
int main(int argc, char* argv[])
{
   ...
   * ***************************************************************************/
   ...
   int   m2c11kx, n2c11kx;
   double *ldatac11kx, *fmidc11kx, *rdatac11kx;

   iC( ddlowrank(nxz,nk,samplec11kxc15kz,eps,npk,lid,rid,mid) );
   m2c11kx=mid.m();
   n2c11kx=mid.n();
   ...
   fmidc11kx  = (double*)malloc(sizeof(double)*m2c11kx*n2c11kx);
   ldatac11kx = (double*)malloc(sizeof(double)*nxz*m2c11kx);
   rdatac11kx = (double*)malloc(sizeof(double)*n2c11kx*nk);

   map2c1c(fmidc11kx, mid, m2c11kx, n2c11kx);

   iC ( samplec11kxc15kz(md,lid,mat) );
   map2c1c(ldatac11kx, mat, nxz, m2c11kx);

   iC ( samplec11kxc15kz(rid,nd,mat) );
   map2c1c(rdatac11kx, mat, n2c11kx, nk);
   
   /********* low rank decomposition of operator: C13*Kx **********/
   int   m2c13kx, n2c13kx;
   double *ldatac13kx, *fmidc13kx, *rdatac13kx;

   iC( ddlowrank(nxz,nk,samplec13kxc35kz,eps,npk,lid,rid,mid) );
   m2c13kx=mid.m();
   n2c13kx=mid.n();
   sf_warning("m2c13kx=%d n2c13kx=%d",m2c13kx, n2c13kx);

   fmidc13kx  = (double*)malloc(sizeof(double)*m2c13kx*n2c13kx);
   ldatac13kx = (double*)malloc(sizeof(double)*nxz*m2c13kx);
   rdatac13kx = (double*)malloc(sizeof(double)*n2c13kx*nk);

   map2c1c(fmidc13kx, mid, m2c13kx, n2c13kx);

   iC ( samplec13kxc35kz(md,lid,mat) );
   map2c1c(ldatac13kx, mat, nxz, m2c13kx);

   iC ( samplec13kxc35kz(rid,nd,mat) );
   map2c1c(rdatac13kx, mat, n2c13kx, nk);

   /********* low rank decomposition of operator: C44*Kx **********/
   int   m2c44kx, n2c44kx;
   double *ldatac44kx, *fmidc44kx, *rdatac44kx;

   iC( ddlowrank(nxz,nk,samplec44kxc35kz,eps,npk,lid,rid,mid) );
   m2c44kx=mid.m();
   n2c44kx=mid.n();
   ...
   fmidc44kx  = (double*)malloc(sizeof(double)*m2c44kx*n2c44kx);
   ldatac44kx = (double*)malloc(sizeof(double)*nxz*m2c44kx);
   rdatac44kx = (double*)malloc(sizeof(double)*n2c44kx*nk);

   map2c1c(fmidc44kx, mid, m2c44kx, n2c44kx);

   iC ( samplec44kxc35kz(md,lid,mat) );
   map2c1c(ldatac44kx, mat, nxz, m2c44kx);

   iC ( samplec44kxc35kz(rid,nd,mat) );
   map2c1c(rdatac44kx, mat, n2c44kx, nk);

   /********* low rank decomposition of operator: C13*Kz **********/
   int   m2c13kz, n2c13kz;
   double *ldatac13kz, *fmidc13kz, *rdatac13kz;

   iC( ddlowrank(nxz,nk,samplecc13kzc15kx,eps,npk,lid,rid,mid) );
   m2c13kz=mid.m();
   n2c13kz=mid.n();
   sf_warning("m2c13kz=%d n2c13kz=%d",m2c13kz, n2c13kz);

   fmidc13kz  = (double*)malloc(sizeof(double)*m2c13kz*n2c13kz);
   ldatac13kz = (double*)malloc(sizeof(double)*nxz*m2c13kz);
   rdatac13kz = (double*)malloc(sizeof(double)*n2c13kz*nk);

   map2c1c(fmidc13kz, mid, m2c13kz, n2c13kz);

   iC ( samplecc13kzc15kx(md,lid,mat) );
   map2c1c(ldatac13kz, mat, nxz, m2c13kz);

   iC ( samplecc13kzc15kx(rid,nd,mat) );
   map2c1c(rdatac13kz, mat, n2c13kz, nk);

   /********* low rank decomposition of operator: C33*Kz **********/
   int   m2c33kz, n2c33kz;
   double *ldatac33kz, *fmidc33kz, *rdatac33kz;

   iC( ddlowrank(nxz,nk,samplecc33kzc35kx,eps,npk,lid,rid,mid) );
   m2c33kz=mid.m();
   n2c33kz=mid.n();
   sf_warning("m2c33kz=%d n2c33kz=%d",m2c33kz, n2c33kz);

   fmidc33kz  = (double*)malloc(sizeof(double)*m2c33kz*n2c33kz);
   ldatac33kz = (double*)malloc(sizeof(double)*nxz*m2c33kz);
   rdatac33kz = (double*)malloc(sizeof(double)*n2c33kz*nk);

   map2c1c(fmidc33kz, mid, m2c33kz, n2c33kz);

   iC ( samplecc33kzc35kx(md,lid,mat) );
   map2c1c(ldatac33kz, mat, nxz, m2c33kz);

   iC ( samplecc33kzc35kx(rid,nd,mat) );
   map2c1c(rdatac33kz, mat, n2c33kz, nk);

   /********* low rank decomposition of operator: C44*Kz **********/
   int   m2c44kz, n2c44kz;
   double *ldatac44kz, *fmidc44kz, *rdatac44kz;

   iC( ddlowrank(nxz,nk,samplecc44kzc15kx,eps,npk,lid,rid,mid) );
   m2c44kz=mid.m();
   n2c44kz=mid.n();
   sf_warning("m2c44kz=%d n2c44kz=%d",m2c44kz, n2c44kz);

   fmidc44kz  = (double*)malloc(sizeof(double)*m2c44kz*n2c44kz);
   ldatac44kz = (double*)malloc(sizeof(double)*nxz*m2c44kz);
   rdatac44kz = (double*)malloc(sizeof(double)*n2c44kz*nk);

   map2c1c(fmidc44kz, mid, m2c44kz, n2c44kz);

   iC ( samplecc44kzc15kx(md,lid,mat) );
   map2c1c(ldatac44kz, mat, nxz, m2c44kz);

   iC ( samplecc44kzc15kx(rid,nd,mat) );
   ...
   * ***************************************************************************/
   ...
   double *txx=(double*)malloc(sizeof(double)*nxz);
   double *tzz=(double*)malloc(sizeof(double)*nxz);
   double *txz=(double*)malloc(sizeof(double)*nxz);
   ...
   double *sx3=(double*)malloc(sizeof(double)*nxz);
   double *sz3=(double*)malloc(sizeof(double)*nxz);
   ...
   for(int it=0;it<ns;it++)
   {
        ...
        fwpvti2de1stlr_rsg_double(ldatapxx1,rdatapxx1,fmidpxx1,ttx,txx,ijkx,ijkz,nx,nz,nxz,nk,m2pxx1,n2pxx1,dxxh,dzzh,akx,akz,1);
        fwpvti2de1stlr_rsg_double(ldatapxz1,rdatapxz1,fmidpxz1,ttxz,txz,ijkx,ijkz,nx,nz,nxz,nk,m2pxz1,n2pxz1,dxxh,dzzh,akx,akz,1);
        fwpvti2de1stlr_rsg_double(ldatapzz1,rdatapzz1,fmidpzz1,ttz,tzz,ijkx,ijkz,nx,nz,nxz,nk,m2pzz1,n2pzz1,dxxh,dzzh,akx,akz,1);
        for(i=0;i<nxz;i++) 
			px3[i] = 2*px2[i] - px1[i] + dt*dt*(ttx[i]+ttxz[i]+ttz[i]);

        /* extrapolation of Vpz-componet */
        fwpvti2de1stlr_rsg_double(ldatapxx2,rdatapxx2,fmidpxx2,ttx,txx,ijkx,ijkz,nx,nz,nxz,nk,m2pxx2,n2pxx2,dxxh,dzzh,akx,akz,1);
        fwpvti2de1stlr_rsg_double(ldatapxz2,rdatapxz2,fmidpxz2,ttxz,txz,ijkx,ijkz,nx,nz,nxz,nk,m2pxz2,n2pxz2,dxxh,dzzh,akx,akz,1);
        fwpvti2de1stlr_rsg_double(ldatapzz2,rdatapzz2,fmidpzz2,ttz,tzz,ijkx,ijkz,nx,nz,nxz,nk,m2pzz2,n2pzz2,dxxh,dzzh,akx,akz,1);
        for(i=0;i<nxz;i++) 
			pz3[i] = 2*pz2[i] - pz1[i] + dt*dt*(ttx[i]+ttxz[i]+ttz[i]);

        /* extrapolation of Vsx-componet */
        fwpvti2de1stlr_rsg_double(ldatasxx1,rdatasxx1,fmidsxx1,ttx,txx,ijkx,ijkz,nx,nz,nxz,nk,m2sxx1,n2sxx1,dxxh,dzzh,akx,akz,1);
        fwpvti2de1stlr_rsg_double(ldatasxz1,rdatasxz1,fmidsxz1,ttxz,txz,ijkx,ijkz,nx,nz,nxz,nk,m2sxz1,n2sxz1,dxxh,dzzh,akx,akz,1);
        fwpvti2de1stlr_rsg_double(ldataszz1,rdataszz1,fmidszz1,ttz,tzz,ijkx,ijkz,nx,nz,nxz,nk,m2szz1,n2szz1,dxxh,dzzh,akx,akz,1);
        for(i=0;i<nxz;i++) 
			sx3[i] = 2*sx2[i] - sx1[i] + dt*dt*(ttx[i]+ttxz[i]+ttz[i]);

        /* extrapolation of Vsz-componet */
        fwpvti2de1stlr_rsg_double(ldatasxx2,rdatasxx2,fmidsxx2,ttx,txx,ijkx,ijkz,nx,nz,nxz,nk,m2sxx2,n2sxx2,dxxh,dzzh,akx,akz,1);
        fwpvti2de1stlr_rsg_double(ldatasxz2,rdatasxz2,fmidsxz2,ttxz,txz,ijkx,ijkz,nx,nz,nxz,nk,m2sxz2,n2sxz2,dxxh,dzzh,akx,akz,1);
        fwpvti2de1stlr_rsg_double(ldataszz2,rdataszz2,fmidszz2,ttz,tzz,ijkx,ijkz,nx,nz,nxz,nk,m2szz2,n2szz2,dxxh,dzzh,akx,akz,1);
        for(i=0;i<nxz;i++) 
